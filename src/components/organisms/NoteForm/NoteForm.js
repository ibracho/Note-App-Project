...
function NoteForm({
  ...
  const [note, setNote] = useState(
    noteForm.isEditState
      ? notes.filter((note) => note.id === parseInt(noteForm.editNoteId))[0]
      : emptyFormState
  );
      ...
      <div className="new-note-container">
        ...
        <div className="new-note">
          <div className="new-note__header">
            ...
            <Button
              buttonType="icon"
              icon={<Icon name="close" height="24" width="24" />}
              clickHandler={() =>
                setNoteFormState({
                  isNotesFormOpen: false,
                  isEditState: false,
          ...
          <InputGroup
            showWordCount={true}
            maxWordsCount={200}
            typedWordsCount={note.description.length}
          ...
          <div className="new-note__input-groups new-note__input-groups--flex-end">
            ...
            {noteForm.isEditState ? (
              <Button
                text="Edit Note"
                clickHandler={onEditNoteHandler}
                buttonType="primary"
              />
            ) : (
              <Button
                text="Add Note"
                clickHandler={onAddNoteHandler}
                buttonType="primary"
              />
            )}
          </div>
