...
function NotesList({
  ...
  let notesList =
    filterNotesBy.text === ""
      ...
      : notes.filter(
          (note) =>
            note.title.toLowerCase().indexOf(filterNotesBy.text.toLowerCase()) >
            -1
        );
  notesList =
    filterNotesBy.category === ""
      ? [...notesList]
      : notesList.filter((note) => note.category === filterNotesBy.category);
  notesList = filterNotesBy.showCompletedOnly
    ? notesList.filter((note) => note.complete === true)
    : notesList;
  ...
  //sort by date
  notesList.sort((p1, p2) => sortFunction(p1, p2));
  // sorty by complete at the end
  notesList.sort((p1, p2) => (p1.complete ? 1 : -1));
      ...
      {notesList.length > 0 ? (
        notesList.map((note) => (
          <Note
            ...
            deleteFormOpenStateHandler={deleteFormOpenStateHandler}
